...
export async function addPosts(newPosts) {
  ...
  if (uniquePosts.length > 0) {
    ...
    console.log(`Added ${uniquePosts.length} new posts. Total posts: ${limitedPosts.length}`);
  }
...
// Function to clean up invalid posts (navigation links, etc.)
export async function cleanupInvalidPosts() {
  const posts = await readPosts();
  let removedCount = 0;
  
  // Import the validation function from scraper
  const { isValidArticleUrl } = await import('./scraper.js');
  
  const validPosts = posts.filter(post => {
    // Check if this is a valid article URL
    const isValid = isValidArticleUrl(post.url, post.title);
    if (!isValid) {
      console.log(`Removing invalid post: ${post.title} - ${post.url}`);
      removedCount++;
    }
    return isValid;
  });
  ...
  if (removedCount > 0) {
    await writePosts(validPosts);
    clearCache(); // Clear cache to ensure fresh data
    console.log(`Cleaned up ${removedCount} invalid posts`);
  }
